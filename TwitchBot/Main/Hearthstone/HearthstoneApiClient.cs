...
namespace TwitchBot.Main.Hearthstone
{
    public class HearthstoneApiClient
    {
        ...
        public HearthstoneApiClient(string clientId, string clientSecret)
        {
            ...
            Console.WriteLine($"BattleNet access token: {_accessToken}");
            LoadBattlegroundsMinionsInfos().Wait();
        ...
        private async Task LoadBattlegroundsMinionsInfos()
        {
            ...
            do
            {
                ...
                while (true)
                {
                    response = _apiClient.Execute(request).Content;
                    if (string.IsNullOrEmpty(response))
                    {
                        Console.WriteLine("BattleNet API returned an empty response. Retrying...");
                        firstTry = false;
                        await Task.Delay(2 * 1000);
                        continue;
                    }

                    if (!firstTry) Console.WriteLine("BattleNet API returned a response with the data. Continue...");
                    break;
                }
            ...
            bool notImplemented = false)
        {
            ...
            if (keyword != MinionKeyword.None)
            {
                ...
                if (keyword == MinionKeyword.Deathrattle)
                    minions = minions.Where(n =>
                        n.AdditionalInfo != null && n.AdditionalInfo.DeathrattleEffect != null);
            }

            if (rarity != MinionRarity.None) minions = minions.Where(n => n.Rarity == rarity);
            // ReSharper disable once InvertIf
            if (!notImplemented)
            {
                var notImplementedIds = new List<int>
                    {2288, 2081, 61028, 61056, 48100, 61047, 65031, 49973, 52502, 64189};
        ...
        private List<AdditionalInfo> GetAdditionalInfos()
        {
            ...
            {
                board.OnMinionLostDivineShield += (s, e) => { minion.AttackPoints += 2; };
            };
            ...
            {
                var elems = GetBattlegroundsMinions(MinionType.Elemental);
            ...
            InfestedWolf.DeathrattleEffect = (minion, owner) =>
            {
                ...
                for (var i = 0; i < 2; i++) owner.Board.SummonBeside(spiderInfo, minion);
            };
            ...
            RatPack.DeathrattleEffect = (minion, owner) =>
            {
                ...
                for (var i = 0; i < numOfRats; i++) owner.Board.SummonBeside(ratInfo, minion);
            };
            ...
            ReplicatingMenace.DeathrattleEffect = (minion, owner) =>
            {
                ...
                for (var i = 0; i < 3; i++) owner.Board.SummonBeside(tokenInfo, minion);
            };
            ...
            RingMatron.DeathrattleEffect = (minion, owner) =>
            {
                ...
                for (var i = 0; i < 2; i++) owner.Board.SummonBeside(tokenInfo, minion);
            };
            ...
            {
                var pirates = GetBattlegroundsMinions(MinionType.Pirate);
                for (var i = 0; i < 3; i++)
                {
